...
//	- 메소드는 다음과 같이 정의한다.
//	(1) public void setTotalScore(int score) : 관객수와 총점을 누적시키는 기능, 평점을 받기만 하면 관객수를 +1하는 기능
//	(2) public String getGrade() : 평점을 구하는 기능, 별찍기는 switch문 이용 / for반복문으로 별찍기처럼도 가능
//	(3) public abstract void getInformation() : 정보를 출력하는 추상메소드 / 자식클래서에서 정의시키기 위해
//	
//	2) Movie 클래스를 정의한다.
//	- 추가로 장르를 필드로 가지며 생성자를 이용하여 값을 초기화한다. +부모 클래스것도 같이
//	- 메소드는 다음과 같이 정의한다.
//	(1) public abstract void getInformation() : 제목, 참여감독 수, 참여배우 수, 관객수, 총점, 평점을 출력하는 기능, / this 안붙여도 됨
//	
...
//	
//	4) 아래와 같은 출력결과가 나오도록 실행코드를 구현한다. // setTotalScore 메소드를 이용하여 값을 저장하면, getInfo를 통해서 아래와 같이 출력될 수 있도록
//	- 출력결과
//		영화제목 : 추격자
//		감독 : 7
//		배우 : 5
